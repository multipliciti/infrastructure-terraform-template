...
Creates the prod environment's infrastructure. These templates are designed to be customized.
...
| default_backend_image | The default docker image to deploy with the infrastructure. Note that you can use the fargate CLI for application concerns like deploying actual application images and environment variables on top of the infrastructure provisioned by this template | string | `quay.io/turner/turner-defaultbackend:0.2.0` | no |
