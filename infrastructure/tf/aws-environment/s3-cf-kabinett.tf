resource "aws_s3_bucket" "kabinett" {
  # Our bucket's name is going to be the same as our site's domain name.
  bucket = var.kabinett_domain_name
  ...
  "Statement":[
    {
      ...
      "Principal": "*",
      "Action":["s3:GetObject"],
      "Resource":["arn:aws:s3:::${var.kabinett_domain_name}/*"]
    }
  ]
}
POLICY
...
resource "aws_cloudfront_distribution" "kabinett_distribution" {
  ...
  origin {
    ...
    domain_name = aws_s3_bucket.kabinett.website_endpoint
    # This can be any name to identify this origin.
    origin_id   = var.kabinett_domain_name
  }
  ...
  default_cache_behavior {
    ...
    target_origin_id       = var.kabinett_domain_name
    min_ttl                = 0
    default_ttl            = 86400
    max_ttl                = 31536000
  ...
  aliases = [var.kabinett_domain_name]
